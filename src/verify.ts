...
//   1. proof signature + hashes        → TAMPERED / SOURCE_MISMATCH
//   2. every fact reproduces exactly   → TAMPERED
//   3. every scene's claim is bound to its fact (numbers + direction)
//                                      → UNSUPPORTED_CLAIM
// A story is VERIFIED only when all layers pass. This is what makes Plotline
// useful to other agents: trust is checkable, not asserted.
...
export function verifyStory(
  ...
  if (spec.proof) {
    ...
    checks.push({
      ...
      ok: true,
      detail: "Skipped: story has no embedded proof bundle (created before proofs, or spec supplied without one). Fact and claim checks still apply.",
    });
  }
