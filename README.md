...
4. **Independent verification.** `verify_story` (MCP tool + `POST /api/verify`) re-runs the ENTIRE analysis from the raw CSV and returns **`VERIFIED` / `TAMPERED` / `UNSUPPORTED_CLAIM` / `SOURCE_MISMATCH`**. Every insight scene also carries a **structured claim** (`factId`, `operation`, `metric`, `unit`, `direction`, `period`, `category`) that is re-derived from the recomputed fact and compared **field-by-field** — so reversed trends, swapped metrics, forged periods, invented percentages, doctored facts and swapped datasets are all caught (see the adversarial test suite).
